...
public class JpaRepoDemo implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        SoftDrink fantaLemon = SoftDrink.builder().name("Fanta_Lemon").rating(2).build();
        SoftDrink seven_up = SoftDrink.builder().name("7up").rating(5).build();


        //save single entity instance
       // fanta = softDrinkRepo.save(fanta);

        // update rating for dr.pepper and coke
        drPepper.setRating(6);
        coke.setRating(5);

        //save multiple entity instances at a time
        List<SoftDrink> insertedSoftDrinks = softDrinkRepo.saveAll(List.of(fanta, coke, drPepper, fantaLemon, seven_up));
       ...
       // }
        ...
        //find all with a rating of 5
        System.out.println("FINDING ALL WITH RATING OF 5");
        softDrinkRepo.findAll(
                        Example.of(
                                //probe soft drink to match results with
                                SoftDrink.builder().rating(5).build(),
